...
-- Explicando o plano de execução de uma consulta simples
EXPLAIN SELECT * FROM usuarios WHERE nome = "Maria";
-- Esta consulta irá retornar todos os registros onde o nome é "Maria".
-- Sem um índice, o banco de dados precisará realizar uma varredura completa na tabela (full table scan).

-- Explicando o plano de execução de uma consulta com junção (join)
EXPLAIN SELECT * FROM usuarios us
INNER JOIN reservas rs
ON us.id = rs.id_usuario
WHERE nome = "Maria";
-- Esta consulta irá retornar todos os registros da tabela 'usuarios' e 'reservas' onde o nome do usuário é "Maria" e o id do usuário corresponde ao id_usuario na tabela reservas.
-- Sem um índice na coluna 'nome', a tabela 'usuarios' será completamente varrida antes de realizar a junção (join).

-- Criando um índice na coluna 'nome' para melhorar a performance da consulta
CREATE INDEX idx_nome ON usuarios (nome);

-- Explicando o plano de execução após a criação do índice
EXPLAIN SELECT * FROM usuarios WHERE nome = "Maria";
-- Após a criação do índice, o banco de dados pode usar o índice 'idx_nome' para localizar rapidamente os registros onde o nome é "Maria", em vez de realizar uma varredura completa na tabela.
